...
% cindy starts
centroidData = []; % Initialize a vector for centroid coordinates
angleData = []; 
timestampData = [];
% cindy ends
...
%for i =200:1:201  
    ...
    [processedImg,angle,timestamp,centroid] = applyOpenCVOperations(img,angleVec(new_idx),time_stamp(new_idx),speedVideo); % You need to define this function
    processedImages{i} = processedImg;

    % cindy starts
    angleData{i,1} = angle;
    timestampData{i,1} = timestamp;
    centroidData{i,1} = centroid(1,1);
    centroidData{i,2} = centroid(1,2);
    % cindy ends
...
for i = 1:1:floor(numFiles/speedVideo)
    ...
    finalImage = uint8(processedImages{i}) * 255;
    % This works 
    finalImage = insertText(finalImage, [500, 25], ['Centroid (Punch): (', num2str(centroidData{i,1}), ', ', num2str(centroidData{i,2}), ')'], ...
        'FontSize', 18, 'TextColor', 'black', 'BoxOpacity', 0);
    finalImage = insertText(finalImage, [20, 25], ['Angle (Part): ', num2str(angleData{i,1}), '°'], ...
        'FontSize', 18, 'TextColor', 'black', 'BoxOpacity', 0);
    finalImage = insertText(finalImage, [20, 500], ['Time: ', num2str(timestampData{i,1}), ' sec'], ...
        'FontSize', 18, 'TextColor', 'black', 'BoxOpacity', 0);
    finalImage = insertText(finalImage, [20, 525], ['Video Speed Up: ', num2str(speedVideo), 'X'], ...
        'FontSize', 18, 'TextColor', 'black', 'BoxOpacity', 0);
...
function [image,angle,timestamp,centroid] = applyOpenCVOperations(image,angle,timestamp,speedVideo)
    %% cindyfx () starts;
    ...
    if isequal(size(B), [3, 1])
        ...
        % text(20, 25, partString, 'FontSize', 14, 'Color', 'k', 'FontWeight', 'bold');
       ...
       % image = insertText(image, [20, 60], ['Testing: '], 'FontSize', 18, 'TextColor', 'black', 'BoxOpacity', 0);
    


    %% Not 3 objects dectected
    else
        % Give an output to centroid
        centroid(1,1) = NaN;
        centroid(1,2) = NaN;
